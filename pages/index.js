...
function Home() {
  ...
  const handleSubmit = (e) => {
    ...
    createCompound(payload).then((response) => {
      const newCompoundId = response.id;
      router.push(`/compound/${newCompoundId}`);
    }).catch((error) => {
  ...
  const totalElementCount = useMemo(() => Object.values(selectElements).reduce((sum, el) => sum + el.count, 0), [selectElements]);
      ...
      <div className="flex-grow-1 d-flex flex-column">
        <div>
          <div className="text-center mb-4">
            ...
            {isCompoundMode && (
            <div className="mt-1">
              <Button onClick={handleSubmit} disabled={totalElementCount < 2 || isLoading} className="mb-2">
                {isLoading ? (
                  <>
                    <Spinner
                      as="span"
          ...
          <div style={{ width: '100%', maxWidth: '800px', margin: '0 auto' }}>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <h2 style={{
                fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '0.5rem', textAlign: 'center', color: 'white',
              }}
              >
                Selected Elements
              </h2>
                ...
                {Object.entries(selectElements).map(([id, el]) => (
                  <div key={id} style={{ width: 'calc(20% - 0.5rem)', margin: '0 0.25rem 0.5rem' }}>
                    <div style={{
                      ...
                      alignItems: 'center',
                      fontSize: '0.75rem',
                      backgroundColor: 'rgba(255, 255, 255, 0.8)', // Added for better visibility
                    }}
                    >
